...
from frappe_bigquery.utils.bq import get_client, table_id_for


class BigQueryQuery(Document):
    """Configuration DocType for exporting from MariaDB to BigQuery."""
    pass
...
def upload(df: pd.DataFrame, dest_table: str):
    ...
    job = client.load_table_from_dataframe(
        df, table_id, job_config=bigquery.LoadJobConfig(schema=_infer_schema(df))
    )
    job.result()
...
def run_query_for_doc(name: str):
    ...
    try:
        ...
        params = {
            "last_sync_ts": doc.last_sync or "1970-01-01 00:00:00",
            "last_sync_id": doc.last_sync_id or 0,
        }
